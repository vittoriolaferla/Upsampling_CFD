...
def scan_csv_files_from_folder(dir_path, recursive=True):
    '''
    Scans CSV files from the given folder.

    Parameters:
        dir_path (str): Path to the directory to scan.
        recursive (bool): If True, scans subdirectories recursively. Default is True.

    Returns:
        List[str]: A sorted list of CSV file paths as strings.
    '''
    # Ensure dir_path is a Path object
    dir_path = Path(dir_path)
    
    # Define the search pattern based on the recursion flag
    pattern = '**/*.csv' if recursive else '*.csv'
    
    # Use glob to find matching files and sort them
    csv_files = sorted([str(file) for file in dir_path.glob(pattern) if file.is_file()])
