...
class GaussianDiffusion:
    ...
    def decode_first_stage(self, z_sample, first_stage_model=None, consistencydecoder=None, downsample=True):
        data_dtype = z_sample.dtype

        if consistencydecoder is None:
            model = first_stage_model
            decoder = first_stage_model.decode if first_stage_model else None
            model_dtype = next(model.parameters()).dtype if model else data_dtype
        else:
        ...
        if first_stage_model is None or decoder is None:
            # If there is no model, perform inverse scaling and return
            if not model_dtype == data_dtype:
                z_sample = z_sample.type(model_dtype)
            # Inverse of scaling applied during encoding
            z_sample = z_sample / (self.scale_factor * 2)
            if not model_dtype == data_dtype:
                z_sample = z_sample.type(data_dtype)
            return z_sample
        else:
            if not model_dtype == data_dtype:
                z_sample = z_sample.type(model_dtype)
           ...
           # with torch.no_grad():
                # Since we have the model, use the decoder
            #out = decoder(z_sample)
            out=z_sample
            if not model_dtype == data_dtype:
                out = out.type(data_dtype)
            # Correct downsampling operation
           # out = F.interpolate(out, scale_factor=self.sf, mode='bicubic')
            #print(out.shape)
    ...
    def training_losses(
        ...
        #print("Tensor of degrated input " + str(z_y.shape))
        #print("Input " + str(z_start.shape))
