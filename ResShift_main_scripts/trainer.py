...
from basicsr.basicsr_resshift.utils import DiffJPEG, USMSharp
from basicsr.basicsr_resshift.utils.img_process_util import filter2D
from basicsr.basicsr_resshift.data.transforms import paired_random_crop
...
class TrainerBase:
    def __init__(self, configs):
        ...
        if self.configs.train.get('use_umass_loss', False):
            ...
            self.umass_loss_weight = self.configs.train['umass_loss_weight']

        if self.configs.train.get('mse_loss_weight', False):
            self.mse_loss_weight =  self.configs.train['mse_loss_weight']
        
        # Initialize the Dissipation loss function if needed
        if self.configs.train.get('use_dissipation_loss', False):
            self.dissipation_loss_fn = Dissipation()
            self.dissipation_loss_weight = self.configs.train.get('dissipation_loss_weight')
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            # Initialize the sharpener if not already done
           ...
           # vel_tensor = data['vel'].cuda()  # Load velocity tensor
            ...
            if random.random() < self.configs.degradation['gaussian_noise_prob']:
                out = random_add_gaussian_noise_pt(
                    out, sigma_range=self.configs.degradation['noise_range'],
                    clip=True, rounds=False, gray_prob=gray_noise_prob)
            else:
                out = random_add_poisson_noise_pt(
                    out, scale_range=self.configs.degradation['poisson_scale_range'],
                    gray_prob=gray_noise_prob, clip=True, rounds=False)

            # ----------------------- The second degradation process ----------------------- #
            if random.random() < self.configs.degradation['second_order_prob']:
                # --- START: Modified Second Degradation (Solution 2) ---

                # Determine random resize scale for the second resize
                updown_type2 = random.choices(
                    ['up', 'down', 'keep'],
                    self.configs.degradation['resize_prob2'],
                )[0]
                if updown_type2 == 'up':
                    scale2 = random.uniform(1, self.configs.degradation['resize_range2'][1])
                elif updown_type2 == 'down':
                    scale2 = random.uniform(self.configs.degradation['resize_range2'][0], 1)
                else:
                    scale2 = 1

                # Get kernel size for the second blur to determine minimum dimension
                k2_size = kernel2.size(-1)
                # The input dimension needs to be at least the kernel size for safe padding/convolution
                min_required_dim = k2_size

                # Calculate the *intended* target size based on randomness
                target_h = int(ori_h / sf * scale2) # Use scale2 here
                target_w = int(ori_w / sf * scale2) # Use scale2 here

                # Ensure the target size is not smaller than the minimum required dimension
                safe_target_h = max(target_h, min_required_dim)
                safe_target_w = max(target_w, min_required_dim)

                # Perform interpolation using the calculated SAFE target size
                mode2 = random.choice(['area', 'bilinear', 'bicubic'])
                out = F.interpolate(
                    out,
                    size=(safe_target_h, safe_target_w), # Use safe dimensions
                    mode=mode2,
                )

                # Apply blur with kernel2 (AFTER the safe resize)
                if random.random() < self.configs.degradation['second_blur_prob']:
                    # This call is now safe because 'out' dimensions are >= k2_size
                    out = filter2D(out, kernel2)

                # Add noise (AFTER safe resize and optional blur)
                gray_noise_prob2 = self.configs.degradation['gray_noise_prob2']
                ...
                # --- END: Modified Second Degradation ---

            # JPEG compression step is removed (as in original code)

            # ----------------------- Final Sinc Filter and Resizing ----------------------- #
            # ... (rest of the sinc filter, resize back, clamp, crop, normalize steps remain the same) ...
            ...
            if self.configs.degradation.resize_back:
                ...
                # temp_sf = self.configs.degradation['sf'] # Not used later, can be removed?
            # else:
                # temp_sf = self.configs.degradation['sf'] # Not used later, can be removed?
            ...
            return {'lq': self.lq, 'gt': self.gt, #'vel': self.vel
             ...
             # Add basic normalization if needed for other dataset types
             if 'lq' in data: data['lq'] = (data['lq'] - 0.5) / 0.5
             if 'gt' in data: data['gt'] = (data['gt'] - 0.5) / 0.5
             return data
    ...
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        ...
        with context():
            ...
            if self.configs.train.get('use_umass_loss', False):
            # Compute physics-based losses using velocities
                umass_loss = self.umass_loss_fn(vel_pred, vel_gt)
                losses['umass_loss'] = self.umass_loss_weight * umass_loss
            if self.configs.train.get('use_dissipation_loss', False):
            # Compute physics-based losses using velocities
                dissipation_loss = self.dissipation_loss_fn(vel_pred, vel_gt)
                losses['dissipation_loss'] =  self.dissipation_loss_weight * dissipation_loss
            if self.configs.train.get('use_vorticity_loss', False):
            # Compute physics-based losses using velocities
                residual_loss = self.vorticity_loss_fn(z0_pred)
                losses['residual loss'] = self.vorticity_loss_weight * (residual_loss/1000)

            total_loss = (
                 self.mse_loss_weight * mse_loss 
                 #+ self.vorticity_loss_weight * (residual_loss/1000)
#                + self.umass_loss_weight * umass_loss 
               #  + self.dissipation_loss_weight * dissipation_loss
            )
            losses['loss'] = total_loss

            # Compute final loss for backpropagation
            loss = total_loss.mean() / num_grad_accumulate
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            latent_downsamping_sf =  1 #2**(len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1)
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, phase='train'):
        '''
        param loss: a dict recording the loss information
        ...
        if self.rank == 0:
            ...
            # Initialize loss_mean and loss_count at specific iterations
            ...
            if self.current_iters % self.configs.train.log_freq[0] == 0:
                ...
                # Average the losses
                for key in self.loss_mean.keys():
                    self.loss_mean[key] /= self.loss_count
                
                # Construct the log string dynamically
                log_str = f'Train: {self.current_iters:06d}/{self.configs.train.iterations:06d}, '
                log_str += 'Losses: '

                for jj, current_record in enumerate(record_steps):
                    log_str += f't({current_record}):'
                    loss_values = []
                    for key in sorted(self.loss_mean.keys()):
                        loss_values.append(f'{key}: {self.loss_mean[key][jj].item():.1e}')
                    log_str += '/'.join(loss_values) + ', '
                
                log_str += f'lr: {self.optimizer.param_groups[0]["lr"]:.2e}'
                self.logger.info(log_str)
                
                # Log metrics (e.g., using TensorBoard or other logging tools)
                self.logging_metric(self.loss_mean, tag='Loss', phase=phase, add_global_step=True)
            
            # Logging images at a different frequency
...
def replace_nan_in_batch(im_lq, im_gt, vel_tensor):
    '''
    Input:
        im_lq, im_gt, vel_tensor: b x c x h x w (or b x h x w for vel_tensor if it has 1 channel)
    '''
    ...
    if nan_mask.any():
        # Get indices of valid samples (where nan_mask is False)
        valid_indices = (~nan_mask).nonzero(as_tuple=False).squeeze(1)
        
        # Ensure there is at least one valid sample
        assert len(valid_indices) > 0, "All samples contain NaNs."
