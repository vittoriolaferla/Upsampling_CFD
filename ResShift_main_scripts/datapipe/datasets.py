...
from utils.utils_ResShift import util_sisr
...
from basicsr.basicsr_resshift.data.realesrgan_dataset import RealESRGANDataset
...
class BaseData(Dataset):
    def __init__(
        ...
        """
        Initializes the BaseData dataset.

        Parameters:
            dir_path (str): Path to the directory containing image files.
            dataroot_vel (str): Path to the directory containing velocity CSV files.
            txt_path (str, optional): Path to a text file listing additional image files.
            transform_type (str): Type of transformation to apply to images.
            transform_kwargs (dict): Keyword arguments for image transformations.
            extra_dir_path (str, optional): Path to an extra directory for additional images.
            extra_transform_type (str, optional): Transformation type for extra images.
            extra_transform_kwargs (dict, optional): Transformation kwargs for extra images.
            length (int, optional): If specified, limits the dataset to this number of samples.
            need_path (bool): If True, includes the image path in the output.
            im_exts (list): List of image file extensions to include.
        ...
        # Scan image files from dir_path
        if dir_path is not None:
            scanned_images = util_common.scan_files_from_folder(dir_path, im_exts, recursive)
            self.image_paths.extend(scanned_images)

        # Scan additional image files from txt_path
        if txt_path is not None:
            scanned_txt = util_common.readline_txt(txt_path)
            self.image_paths.extend(scanned_txt)

        # Scan CSV files from dataroot_vel
        if dataroot_vel is not None:
            scanned_vel = util_common.scan_csv_files_from_folder(dataroot_vel, recursive)
            self.vel_paths.extend(scanned_vel)

        # Optionally limit the dataset length
        if length is not None:
            # Sample without replacement for images
            if len(self.image_paths) > length:
                self.image_paths = random.sample(self.image_paths, length)
            # Sample without replacement for velocities
            if len(self.vel_paths) > length:
                self.vel_paths = random.sample(self.vel_paths, length)

        # Define dataset length as the maximum of image and velocity counts
        self.length = max(len(self.image_paths), len(self.vel_paths))

        self.need_path = need_path
        self.transform = get_transforms(transform_type, transform_kwargs)

        # Handle extra directory transformations
        self.extra_dir_path = extra_dir_path
        if self.extra_dir_path is not None:
            assert extra_transform_type is not None, "extra_transform_type must be provided if extra_dir_path is set."
    ...
    def __len__(self):
        ...
        The length is defined as the maximum of the number of images and velocity files.
        This ensures that all data points are accessible, with `None` used where data is missing.
        """
        return self.length
    ...
    def __getitem__(self, index):
        ...
        Returns:
            dict: A dictionary containing image data, velocity data, and optional additional information.
        """
        out = {}
        ...
        if index < len(self.image_paths):
            ...
            im_base = util_image.imread(img_path, chn='rgb', dtype='float32')
            im_target = self.transform(im_base)
            out['image'] = im_target
            out['lq'] = im_target  # Assuming 'lq' is a duplicate; adjust if necessary

            # Handle extra directory if provided
            if self.extra_dir_path is not None:
                im_path_extra = Path(self.extra_dir_path) / Path(img_path).name
                if im_path_extra.exists():
                    im_extra = util_image.imread(im_path_extra, chn='rgb', dtype='float32')
                    im_extra = self.extra_transform(im_extra)
                    out['gt'] = im_extra
                else:
                    out['gt'] = None  # Or handle as desired

            # Include the image path if needed
            if self.need_path:
                out['path'] = img_path
        else:
            # If no image exists for this index
            out['image'] = None
            out['lq'] = None
            if self.extra_dir_path is not None:
                out['gt'] = None
    ...
    def reset_dataset(self, length=None):
        """
        Resets the dataset by sampling a new subset of data.

        Parameters:
            length (int, optional): If provided, samples that many images and velocities.
                                     Otherwise, retains the current dataset size.
        """
        if length is not None:
            # Sample without replacement for images
            if len(self.image_paths) > length:
            ...
            # If no length is provided, reset to current lengths
            self.length = max(len(self.image_paths), len(self.vel_paths))

    def _load_velocity(self, vel_path):
        """
        Loads and processes the velocity data from a CSV file.

        Parameters:
            vel_path (str): Path to the CSV file.

        Returns:
            dict: Processed velocity data.
        """
        import pandas as pd

        try:
            df = pd.read_csv(vel_path)
            # Example processing: convert to a dictionary with lists
            vel_data = df.to_dict(orient='list')
            return vel_data
        except Exception as e:
            print(f"Error loading velocity data from {vel_path}: {e}")
            return None
