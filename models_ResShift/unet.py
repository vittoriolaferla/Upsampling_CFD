    ...
    def forward(self, x, emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                #print("Timestep dim" + str(x.shape))
                x = layer(x)
        return x
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        if cond_lq and lq_size != image_size:
            ...
            self.factor_bicubic= image_size/lq_size
        else:
            feature_extractor = []
    ...
    def forward(self, x, timesteps, lq=None, mask=None):
        ...
        if lq is not None:
            ...
            #print("Input shape at U-Net " +str(x.shape))
            #print("Low Quality shape " + str(lq.shape))
            lq = F.interpolate(lq, scale_factor=self.factor_bicubic, mode='bicubic')
            x = th.cat([x, lq], dim=1)
