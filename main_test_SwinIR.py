...
def define_model(args):
    ...
    # 004 grayscale image denoising
        ...
        model = net(upscale=args.scale, in_chans=3, img_size=args.training_patch_size, window_size=4,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='nearest+conv', resi_connection='1conv')
        param_key_g = 'params'
...
def get_image_pair(args, path):
    ...
    # 004 grayscale image denoising (load gt image and generate lq image on-the-fly)
        ...
        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}', cv2.IMREAD_GRAYSCALE).astype(
            np.float32) / 255.
